 ...
 */
class RemoveBlock extends \Magento\Framework\View\Element\Template
{
    ...
    public function __construct(
        ...
        if ($this->helper->getModuleStatus()) {
            if (!$this->helper->subscriptionStatus()) {
                ...
                $this->setTemplate("hide/addtocart.phtml");
                $layout = $this->getLayout();
                $block = $layout->getBlock('product.info.addtocart.additional'); // block name
                $layout->unsetElement('product.info.addtocart.additional');
                $block = $layout->getBlock('product.info.addtocart'); // block name
                $layout->unsetElement('product.info.addtocart');
                $block = $layout->getBlock('product.price.tier'); // block name
